#!/usr/bin/env python3
"""
Fine-tune New Medical Embedding Models with Optimized Contrastive Learning.

Models to fine-tune:
1. FremyCompany/BioLORD-2023 (Best zero-shot: Pearson 0.2234)
2. abhinand/MedEmbed-small-v0.1 (Pearson 0.1620)
3. ncbi/MedCPT-Query-Encoder (Pearson 0.1257)
4. cambridgeltl/SapBERT-from-PubMedBERT-fulltext (Pearson 0.1164)
5. EMBO/soda-vec-negative-sampling (Pearson 0.1017)
6. ncbi/MedCPT-Article-Encoder (Pearson 0.0959)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(Path(__file__).resolve().parent.parent / 'logs' / '16_finetune_new_embeddings.log'))
    ]
)
...
MODELS_TO_FINETUNE = [
    {
        ...
        'batch_size': 16,
        'prefix': '',
        'description': 'BioLORD 2023 - Best zero-shot (Pearson 0.2234)',
    ...
    },
    {
        ...
        'batch_size': 32,
        'prefix': '',
        'description': 'MedEmbed Small v0.1 (Pearson 0.1620)',
        'use_sentence_transformer': True
    },
    {
        ...
        'batch_size': 16,
        'prefix': '',
        'description': 'MedCPT Query Encoder (Pearson 0.1257)',
        'use_sentence_transformer': True
    },
    {
        ...
        'batch_size': 16,
        'prefix': '',
        'description': 'SapBERT from PubMedBERT (Pearson 0.1164)',
        'use_sentence_transformer': True
    },
    {
        'name': 'EMBO/soda-vec-negative-sampling',
        'short_name': 'soda-vec',
        'dim': 768,
        'lr': 2e-5,
        'batch_size': 16,
        'prefix': '',
        'description': 'SODA-vec Negative Sampling (Pearson 0.1017)',
        'use_sentence_transformer': True
    },
    {
        ...
        'batch_size': 16,
        'prefix': '',
        'description': 'MedCPT Article Encoder (Pearson 0.0959)',
        'use_sentence_transformer': True
    },
]
...
class ImprovedContrastiveLoss(nn.Module):
    ...
    def forward(
        ...
        try:
            q_expanded = q_emb.unsqueeze(0).expand(n, -1)
            d_q_sims = F.cosine_similarity(d_embs, q_expanded, dim=1)
...
class FineTuner:
    ...
    def _save_checkpoint(self, name: str):
        """Save checkpoint."""
        output_dir = (
            Path(__file__).parent.parent / 'models' / 'finetuned_new' / 
...
def main():
    """Main function."""
    logger.info("=" * 70)
    logger.info("FINE-TUNING NEW MEDICAL EMBEDDING MODELS")
    logger.info("=" * 70)
    logger.info(f"Started: {datetime.now().isoformat()}")
    
    project_dir = Path(__file__).parent.parent
    output_dir = project_dir / 'outputs' / 'results' / 'finetuned_new'
    output_dir.mkdir(parents=True, exist_ok=True)
